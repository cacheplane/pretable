...
export const pmsColumns: readonly ScenarioColumn[] = Object.freeze([
  ...identityColumns,
  ...numericColumnIds.map((id): ScenarioColumn => ({
    id,
    header: humanize(id),
    wrap: false,
    widthPx: 110,
...
export const pmsRoles: ScenarioRoles = Object.freeze({
  ...
  stream: Object.freeze({
    ...
    derivedColumnIds: Object.freeze([
      "marketValue",
      "unrealizedPnl",
      "dayPnl",
      "dayChangePct",
...
function buildPmsRow(seed: number, rowIndex: number): ScenarioRow {
  ...
  const sector =
    PMS_SECTORS[
      Math.floor(rowIndex / PMS_STRATEGIES.length) % PMS_SECTORS.length
    ]!;
  ...
  const notes = Array.from({ length: fragmentCount }, () =>
    pick(noteFragments),
  ).join(" ");
