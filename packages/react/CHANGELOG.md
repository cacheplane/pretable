...
- A windowed grid now redraws its leading spacer and scroll extent when a count ([#569](https://github.com/cacheplane/pretable/pull/569))
...
- A rejected `rows` update no longer leaves the grid describing itself with the ([#561](https://github.com/cacheplane/pretable/pull/561))
  array it refused.
...
- `usePretable` — and `PretableSurface`, which routes its `rows` through it — ([#557](https://github.com/cacheplane/pretable/pull/557))
...
- An invalid `query` arriving on the prop after mount is a rejected write, not a ([#553](https://github.com/cacheplane/pretable/pull/553))
  destroyed grid.
