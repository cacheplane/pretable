...
export function DrawerHandle() {
  ...
  const { isOpen, isUpgraded, open } = useDrawer();
  return (
    <button
      ...
      // The handle paints from the SSR markup, but `open` is only attached once
      // this component hydrates; until then a press is accepted and dropped.
      // Publish that readiness so it is observable instead of guessed at.
      data-hydrated={isUpgraded ? "true" : "false"}
