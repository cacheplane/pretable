...
### One token to restate under Material

There is one exception, and it is worth knowing before you ship. `material.css` derives most of its grid controls from other tokens, which is why its dark block can be short and still come out right: `--pretable-checkbox-checked-bg` is `var(--pretable-accent)`, so it follows the accent into dark on its own. Its partner is a literal — `--pretable-checkbox-checked-fg: #fff` — and the dark block does not restate it, so a checked checkbox draws a white mark on the dark scheme's light-blue primary at **1.70:1**.

That is the design language showing through rather than an oversight. In M3 the pair is `primary` / `on-primary`, and `on-primary` is light in one scheme and dark in the other, so no single literal can serve both. White is the correct value at `:root`, where it sits on the light primary at 6.47:1.

Restate it in your own block after the import:
...
[data-theme="dark"] {
  /* M3's baseline dark `on-primary` — 7.73:1 on the dark primary. */
  --pretable-checkbox-checked-fg: #003258;
}
```

`pretable.css` is not affected. Its dark block restates every color token it declares, literals included, precisely so a value tuned for light mode cannot survive into dark unnoticed — the discipline [Custom themes](/docs/theming/custom-themes) recommends for your own.
