...
When overrides grow beyond a handful of tokens, switch from override-on-top-of-existing to authoring a theme file from scratch. The contract is straightforward: define all 50 `--pretable-*` tokens at `:root`. Optionally add `[data-density="..."]` and `[data-theme="dark"]` blocks for runtime variants.
...
Here is a complete, working theme — every one of the 50 tokens, a `[data-theme="dark"]` block, and both non-default density tiers. Its selectors are scoped to a wrapper class rather than written at a bare `:root`, purely so this one demo doesn't re-theme the rest of this documentation page around it; the callout beneath the code explains the substitution. Copy `brand.css` as your starting point — every focus-marked line below is one this page walks through by name in the sections that follow.

<Example id="custom-theme" />

The excerpt below shows the same shape in miniature: the surface, text, line, state, accent, density, and typography groups, with the editing, semantic-ramp, elevation, icon, and grid-control groups left out for length. The [token reference](/docs/theming/token-reference) has all 50 with descriptions and types.
