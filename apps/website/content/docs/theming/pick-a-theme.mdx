...
- **One functional hue.** `--pretable-accent` means "interactive or selected" and nothing else — never a surface, never chrome text. It is a restrained blue rather than a brand color, so the grid reads as part of your application instead of competing with it.
...
Because all three define the same 49 tokens at `:root`, nothing else in your stylesheet has to change — anything you wrote against `var(--pretable-*)` keeps resolving.
