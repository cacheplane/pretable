...
One token needs a hand if you ship Material in dark mode. Its dark block leaves `--pretable-checkbox-checked-fg` at the light value `#fff`, while `--pretable-checkbox-checked-bg` re-resolves to the dark primary — so a checked checkbox draws a white mark on light blue at 1.70:1. That is M3's roles showing through rather than an oversight: the pair is `primary` / `on-primary`, and `on-primary` is genuinely a different color in each scheme, so no single literal serves both. One line fixes it, and [Light / dark switching](/docs/theming/light-dark) has the snippet.
