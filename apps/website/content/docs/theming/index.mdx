...
<Example id="theming-composition-order" />
...
│    --pretable-accent: #3f5a7a;      │   54 tokens defined
│    ...                              │
│  }                                  │
└─────────────────────────────────────┘
                 ▼ writes to
...
**Layer 1 — theme tokens.** A theme file declares all 54 `--pretable-*` tokens at `:root`. This is the single source of truth for what the grid looks like. Three ship: `themes/pretable.css` is the house theme and the default, and `themes/excel.css` and `themes/material.css` are compatibility skins for apps whose design language is already one of those. See [Pick a theme](/docs/theming/pick-a-theme).
...
## The 54-token contract

Pretable's public token contract has 54 tokens, grouped:
...
- **Lines and radii (9):** `rule`, `rule-vertical`, `rule-width`, `rule-strong`, `rule-header`, `frame`, `radius-frame`, `radius`, `radius-control`
...
- **Elevation (4):** `shadow-overlay`, `shadow-card`, `shadow-header`, `seam-color`
