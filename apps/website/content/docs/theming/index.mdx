...
> Two of the density tokens (`--pretable-row-height` and `--pretable-header-height`) are read by the engine in JavaScript, because the virtualizer needs them as numbers. The other 22 are CSS-only. `@pretable/ui`'s [`getDensityHeights`](/docs/grid/density-helpers) reads the same two.
