...
The override story is plain CSS cascade. Pick a theme, redefine any of its 54 tokens in your own stylesheet, and your values win because they load after the theme.

<Example id="token-override-live" />
...
The container and the lifted surfaces round separately. `--pretable-radius-frame` is the container corner: the `pretable` theme ships `0` (it is frameless, so there is no edge to round), Material `12px`, and Excel `0` for sharp spreadsheet corners. `--pretable-radius` rounds the popovers, the kit listbox, and the group chips: `10px` under `pretable`. To move either without writing a new theme:
...
:root {
  --pretable-radius-frame: 4px;
  --pretable-radius: 4px;
}
```

The grid container's outer corners and every popover now reflect your values. Cells stay square — `--pretable-radius-frame` applies only to the outermost wrapper, and under the `pretable` theme it has little visible effect until you also give the container an edge with `--pretable-frame`. Small affordances (menu items, the funnel and ⋮ buttons, the chip remove target) take `--pretable-radius-control` instead, because the container radius applied to a 14–18px control rounds it into a circle.
...
The 54 tokens group by purpose. When overriding, think in groups:

- **Surfaces** drive cell, pinned-cell, group-row, header, toolbar, and popover backgrounds.
- **Text** drives readable content color.
- **Lines and radii** drive gridlines, the container frame, and corner rounding. The horizontal and vertical rules are separate tokens, so a theme can drop the column cage without losing row separation.
...
- **Elevation** drives popover shadows, the container's card shadow, the sticky header's seam while scrolled, and the frozen-column seam.
...
- **Scope overrides if needed** — to override per-section of your app, scope to a class or attribute: `.dashboard { --pretable-accent: red; } .reports { --pretable-accent: blue; }`. The grid inside each scope reads the locally scoped value. For popups, use an [overlay host in the same scope](/docs/grid/components#overlay-scope); the default body portal cannot inherit a section’s local CSS.

## Where to go next

- [Token reference](/docs/theming/token-reference) — the canonical 54-token table.
