...
These tokens control the visual appearance of interactive grid elements: row-selection checkboxes, cell-range selection overlays, column resize handles, and column reorder drag-and-drop indicators. By default they all derive from the theme's `--pretable-accent`, `--pretable-bg-grid`, and `--pretable-rule-strong`, so overriding those three upstream tokens recolors the controls coherently without touching this group individually.

| Token                               | Description                                                  | Type   | Excel                                         | Material 3 (light)                            |
| ----------------------------------- | ------------------------------------------------------------ | ------ | --------------------------------------------- | --------------------------------------------- |
| `--pretable-selection-bg`           | Range-selection overlay background (`[aria-selected]` cells) | color  | `color-mix(in srgb, #107c41 8%, transparent)` | `color-mix(in srgb, #6750a4 8%, transparent)` |
| `--pretable-checkbox-bg`            | Row-select checkbox background                               | color  | `#ffffff` (= `bg-grid`)                       | `#fef7ff` (= `bg-grid`)                       |
| `--pretable-checkbox-border`        | Row-select checkbox border                                   | color  | `#a6a6a6` (= `rule-strong`)                   | `#79747e` (= `rule-strong`)                   |
| `--pretable-checkbox-checked-bg`    | Row-select checkbox background when checked                  | color  | `#107c41` (= `accent`)                        | `#6750a4` (= `accent`)                        |
| `--pretable-checkbox-checked-fg`    | Row-select checkmark color                                   | color  | `#ffffff`                                     | `#ffffff`                                     |
| `--pretable-resize-handle`          | Column resize handle (idle)                                  | color  | `transparent`                                 | `transparent`                                 |
| `--pretable-resize-handle-hover`    | Column resize handle (hover/dragging)                        | color  | `#107c41` (= `accent`)                        | `#6750a4` (= `accent`)                        |
| `--pretable-reorder-ghost-bg`       | Column drag ghost background                                 | color  | `#f3f3f3` (= `bg-header`)                     | `#f3edf7` (= `bg-header`)                     |
| `--pretable-reorder-ghost-shadow`   | Column drag ghost shadow                                     | shadow | `0 4px 12px rgb(0 0 0 / 0.12)`                | `0 4px 12px rgb(0 0 0 / 0.12)`                |
| `--pretable-reorder-drop-indicator` | Column drop-position indicator                               | color  | `#107c41` (= `accent`)                        | `#6750a4` (= `accent`)                        |
