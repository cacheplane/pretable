...
description: "The 54-token --pretable-* surface, with shape, default, and purpose for each."
nav: Theming
---

Pretable's public theming surface is 54 CSS variables, all `--pretable-*` prefixed. Each theme defines all 54 at `:root`. Three themes ship preset values — `pretable`, the house theme and the documented default, plus the Excel and Material 3 compatibility skins. Consumers override individual tokens at `:root` in their own stylesheet (see [Override tokens](/docs/theming/override-tokens)).

> Three tokens — `--pretable-row-height`, `--pretable-header-height`, and `--pretable-group-panel-height` — are read by the engine in JavaScript, because it needs them as numbers to lay out rows, the sticky header, and the group panel. The other 51 are CSS-only. The engine resolves the three against the grid's own element, so they inherit from wherever you set them — `:root`, or a wrapper — like every other token. Of the three, the first two are also readable from your own code via [`getDensityHeights`](/docs/grid/density-helpers) in `@pretable/ui`, which takes the element to resolve against.
...
| `--pretable-bg-group-row` | Group header row background                       | color | `#fafafb`          | `var(--pretable-bg-header)` | `var(--pretable-bg-header)` |
| `--pretable-bg-header`    | Header row background                             | color | `#f7f7f9`          | `#f3f3f3`                   | `#f3f3f3` (`N96`)           |
| `--pretable-bg-toolbar`   | Toolbar + status/pagination bar background        | color | `#f7f7f9`          | `#f3f3f3`                   | `#f3f3f3`                   |
...
| `--pretable-text-header` | Header text color                                              | color | `#6b6b76`          | `#5c5c5c` | `#49494c` (`on-surface-variant`) |
...
| Token                       | Description                                                                                             | Type   | pretable (default) | Excel                                   | Material 3 (light)                      |
| --------------------------- | ------------------------------------------------------------------------------------------------------- | ------ | ------------------ | --------------------------------------- | --------------------------------------- |
| `--pretable-rule`           | Horizontal rule color — the hairline between rows                                                       | color  | `#ececf0`          | `#d4d4d4`                               | `#cccccc` (`outline-variant`)           |
...
| `--pretable-rule-strong`    | Edge of lifted surfaces (menus, popovers, editor); the frame and header rule in the compatibility skins | color  | `#c2c2cb`          | `#a6a6a6`                               | `#797979` (`outline`)                   |
...
| `--pretable-radius-frame`   | Container corner radius                                                                                 | length | `0`                | `0`                                     | `12px` (M3 medium shape)                |
| `--pretable-radius`         | Radius of lifted surfaces and chips                                                                     | length | `10px`             | `0`                                     | `12px` (M3 medium shape)                |
...
> `--pretable-rule-width` governs only the per-cell row and column dividers. Popover, menu, and editor borders stay `1px` regardless, and the container edge is whatever `--pretable-frame` says. That token is a whole border shorthand, so a theme drops the frame with `0` or draws one with `1px solid var(--pretable-rule-strong)`.

> `--pretable-radius-frame` rounds the container, and nothing else; `--pretable-radius` rounds what is lifted or chip-shaped — the popovers, the kit listbox, and the group chips. The two are separate so a frameless theme can keep square container corners (there is no edge to round) while its popovers still stay rounded. Applying either to a 14–18px control rounds that control into a circle; `--pretable-radius-control` is the separate, smaller step those affordances take: menu items, the funnel and ⋮ buttons, and the chip remove target.
...
| `--pretable-bg-selected`   | Selected-option background (kit lists + date day) | color | `#dfe6fb`                | `rgba(16, 124, 65, 0.10)` | `#d1e4ff` (`secondary-container`)            |
...
> `--pretable-bg-selected` does **not** fill a selected cell, despite the name. The grid stylesheet reads it in exactly two places, and neither is a cell: the shared `[data-pretable-option][aria-selected="true"]` rule — which paints the chosen entry in every list the kit draws, the filter dialog's and the tool panel's operator and column pickers, the grouping pane's aggregate picker, and the enum cell editor alike — and the date picker's selected day. The fill behind a selected cell or range is `--pretable-selection-bg`, in Grid controls below — it is a translucent tint, drawn as a background layer so a pinned cell keeps its opaque fill underneath. `--pretable-text-selected` is the one of the pair that does span both: it colors selected cell text as well as those two marks.
...
| `--pretable-shadow-card`    | Container elevation                                 | shadow | `none`                                                                     | `none`                         | `none`                         |
...
| Token                               | Description                                 | Type  | pretable (default)        | Excel                                                        | Material 3 (light)                                           |
| ----------------------------------- | ------------------------------------------- | ----- | ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `--pretable-selection-bg`           | Range-selection overlay background          | color | `rgba(37, 84, 207, 0.07)` | `color-mix(in srgb, var(--pretable-accent) 8%, transparent)` | `color-mix(in srgb, var(--pretable-accent) 8%, transparent)` |
| `--pretable-checkbox-bg`            | Row-select checkbox background              | color | `#ffffff`                 | `var(--pretable-bg-grid)`                                    | `var(--pretable-bg-grid)`                                    |
| `--pretable-checkbox-border`        | Row-select checkbox border                  | color | `#8e8e99`                 | `var(--pretable-rule-strong)`                                | `var(--pretable-rule-strong)`                                |
