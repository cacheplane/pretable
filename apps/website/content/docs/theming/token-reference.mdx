...
| `--pretable-rule-strong`    | Edge of lifted surfaces (menus, popovers, editor) | color  | `#c2c2cb`          | `#a6a6a6`                               | `#797979` (`outline`)                   |
...
> `--pretable-rule-width` governs only the per-cell row and column dividers. Popover, menu, and editor borders stay `1px` regardless, and the container edge is whatever `--pretable-frame` says — a whole border shorthand, so a theme can drop the frame with `0` or draw one with `1px solid var(--pretable-rule-strong)`.
