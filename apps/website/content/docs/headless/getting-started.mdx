...
`getState()` is referentially stable until the committed snapshot or observable status changes. The example goes one step further and selects `.snapshot` directly rather than the whole state — see [Snapshot & subscribe](/docs/headless/state-model) for why that's what keeps a rebuild from re-rendering the table on every slice.
