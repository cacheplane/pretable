...
```ts
grid.setSort("latencyMs", "asc"); // single-column sort "asc" | "desc"
grid.setSort(null, null); // clear the sort

grid.replaceSort([
  { columnId: "team", direction: "asc" },
  { columnId: "latencyMs", direction: "desc" },
]); // atomic multi-sort — index = priority
grid.replaceSort([]); // clear the sort
...
`setSort(columnId, direction)` takes a `PretableSortDirection` (`"asc"`, `"desc"`, or `null`); pass `null` for both to clear. It replaces the whole sort list with a single entry. `replaceSort(entries)` swaps in a complete ordered `PretableSortEntry[]` — later entries break ties left by earlier ones; entries for unknown or `sortable: false` columns are dropped. Sorting and filtering both re-derive `visibleRows`.
...
grid.setColumnPinned("name", "left"); // pin to an edge: "left" | "right"
...
It is a report, not a command: calling it does not scroll anything. `<PretableSurface>` feeds it from its scroll container's `onScroll`, which also covers the scrolling the surface performs itself to [follow keyboard focus](/docs/grid/keyboard#scroll-follows-focus) — the surface assigns `scrollTop` / `scrollLeft`, the browser fires `scroll`, and the resulting position arrives back here. If you render your own DOM, wire it the same way and drive the scroll offsets yourself.
