...
type PretableVisibleRowRef<TRowId> =
  { kind: "data"; rowId: TRowId } | { kind: "group"; groupId: string };

interface PretableSurfaceFocusState<TRowId = string> {
...
interface PretableSurfaceState {
  ...
  focus?: PretableSurfaceFocusState;
...
- `PretableSurfaceMessages` — overrides for the polite `aria-live` announcements (select-all, and copy/paste success/failure). `pasteAnnouncement` is one function rather than one per outcome because a clean, partial and wholly rejected paste are the same sentence at different counts, and clipping is orthogonal to all three. See [Paste](/docs/grid/paste#announcements).
...
interface UsePretableOptions<TRow> {
  ...
  onFocusChange?: (next: PretableSurfaceFocusState) => void;
}
