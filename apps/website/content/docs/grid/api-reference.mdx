...
`renderSnapshot.columns` is the column layout — left offset and width per column, accounting for pinned positioning. Left-pinned columns come first, then the visible scrollable window, then the right-pinned columns; a right-pinned entry carries `right` (its offset from the viewport's right edge) instead of a meaningful `left`.
