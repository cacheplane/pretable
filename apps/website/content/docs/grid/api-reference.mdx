...
The tuple preserves row, column-ID, value, filter, editor, formatter, and
...
`isValidDateValue(value)` narrows an exact, Gregorian `YYYY-MM-DD` string.
...
`@pretable/core`. See [Date formatting](/docs/grid/date-formatting) for storage,
...
- `isValidDateValue` / `PretableDateFormatOptions` — strict calendar-date validation and native display options.
