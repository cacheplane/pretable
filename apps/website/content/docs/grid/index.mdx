...
If you need any of the above, jump to Path 2. See [\<Pretable\> component](/docs/grid/pretable-component) for the full props reference.
...
See [\<PretableSurface\> component](/docs/grid/pretable-surface) for the configuration hub, and [Custom rendering](/docs/grid/custom-rendering) for cell/header render hooks with working examples.
...
The other 47 tokens in [the theming contract](/docs/theming/token-reference) are CSS-only — they style the data-attribute selectors but don't enter JavaScript.

`getDensityHeights` from `@pretable/ui` reads the first two of the three into your own code — row and header height, not the group panel. See [Density helpers](/docs/grid/density-helpers) for its fallbacks and SSR behaviour.
...
- [\<Pretable\> component](/docs/grid/pretable-component) — drop-in recipe with the 4-prop API.
...
- [Row grouping and aggregation](/docs/grid/grouping) — grouping, aggregates, expansion, and the group panel.
