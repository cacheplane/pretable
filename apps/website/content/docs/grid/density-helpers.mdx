...
These fallbacks match `@pretable/ui`'s density snapshot contract and provide deterministic geometry when CSS cannot be read, including during server rendering and the browser hydration pass.

### SSR safety

The hook is SSR-safe. During server rendering and the browser hydration pass, it uses explicit prop overrides or deterministic fallbacks so the geometry matches. Immediately after hydration, the client snapshot adopts CSS-resolved values. Later theme or density changes are observed through `MutationObserver`.
