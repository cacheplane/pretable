...
Column-level `render` and `renderEditor` callbacks are useful when presentation belongs with one typed column. Surface-level hooks are better for a design-system wrapper. Shared presentations such as `PretableBadge`, `PretableDelta`, and `PretableEntity` keep semantic color, contrast, and secondary text aligned with the active theme.
