...
description: "usePretableModel returns the engine's full state: the grid model (with interaction methods), the snapshot (sort, filters, selection, focus), the render snapshot (which rows to render at what positi…"
nav: Grid
order: 4
---
          ...
          return (
            <div
              ...
              onClick={() => {
                ...
                grid.setFocus({ rowId: row.id, columnId: columns[0]?.id ?? null });
              }}
            >
              {renderSnapshot.columns.map((col) => {
