...
- **Pinned columns sticky positioning** — apply `position: sticky; left: ${col.left}px` to left-pinned cells and `position: sticky; right: ${col.right}px` to right-pinned ones. `renderSnapshot.columns` already carries `pinned` plus the resolved `left` / `right` offsets, which is exactly what `<PretableSurface>` positions against.
