...
A range that spans [group rows](/docs/grid/grouping) serializes them too: the group's label in the derived group column, each column's `formatAggregate` output where it has an aggregate, and an empty cell everywhere else.
