...
| `locale?`          | `Intl.LocalesArgument \| undefined`                   | public presentation locale used by native number formatting                          |
| `scope`            | `"all" \| "loaded"` &nbsp; _(@experimental)_          | whether copied group aggregates fold every matching record or only the loaded window |
