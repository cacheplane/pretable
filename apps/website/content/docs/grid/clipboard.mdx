...
Writing two flavors at once needs `ClipboardItem`. Where that is missing — an older browser, a non-DOM test environment — the surface falls back to `navigator.clipboard.writeText` and only the TSV reaches the clipboard. The serializer still produces both; the write is what degrades.
