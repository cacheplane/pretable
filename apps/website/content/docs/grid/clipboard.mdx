...
Because copy quotes per the [escaping rule](#escaping) above, the paste path has to unquote the incoming text — splitting rows and columns on raw newlines and tabs — before any per-column `parseFromCopy` sees it. `parseFromCopy` will receive the already-unquoted cell text, so it stays a pure value parser and never has to know about the TSV encoding.
