...
Copy the 2x2 block, then paste it into the grid below. A single cell writes it once; select **Symbol + Qty across the first 4 rows** — an exact multiple of the block — to see it tile; anchor on the last row to see the overflow clipped, reported in the status line, and appended as a new row through the same `onPasteCapture` + `parseTsv` recipe above:
