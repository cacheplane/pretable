...
Try a mixed inventory update below. Copy the three quantities and paste into
the first **Quantity** cell: `24` applies, `-5` fails the nonnegative rule, and
`abc` fails number parsing. Pens and Folders keep their previous quantities,
with a reason shown for each rejected value. Choose **Use corrected values**,
then copy and paste again to apply all three rows.

<Example id="paste-validation" />

The example applies accepted cells by row id against current state and renders
`rejected` as a readable list. A compact status summary uses the same payload:
