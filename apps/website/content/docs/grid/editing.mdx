...
`saving` and `error` are the two phases text on this page can only assert, not show — the field visibly goes read-only and `aria-busy` while a commit is in flight, and a rejection leaves the editor open with an inline message. Try it below: every column is a different [typed editor](#typed-editors), and editing **Quantity** to a negative number rejects after a delay so you can watch both phases happen.
...
The [live example under Lifecycle](#lifecycle) is this page's worked example: `editable` columns, a controlled `rows` array, and `onRowChange` publishing the complete proposed row — the same shape built up through this page, plus every [typed editor](#typed-editors) and the full async lifecycle in one file. Open its Code tab to read or copy `AsyncEditingGrid.tsx`.

Click a cell, press `Enter` (or just start typing), edit, and press `Enter` again — `onRowChange` emits a proposal, the demo's own state publishes it, and the new row flows back into the grid.
