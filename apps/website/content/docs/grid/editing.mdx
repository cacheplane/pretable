...
The editor seeds itself from the cell value and normalises it to ISO. It deliberately accepts only values whose day is **unambiguous**:
...
A zoned datetime resolves to its **UTC** day, matching how the filter engine buckets the same value — so a cell reads the same day for every viewer, and editing it can't move a row out from under its own filter.

<Callout type="warning">
  A **zone-less** datetime — `"2026-08-06T00:00:00"`, the shape many JSON and
  SQL backends emit — is deliberately **not** accepted, and neither are locale
  strings like `"08/06/2026"`. Without a zone they mean a different day
  depending on who is looking, so the editor refuses to guess: such a cell opens
  showing its raw text and is rejected on commit with "Use YYYY-MM-DD".

Filtering is currently more forgiving here and _does_ resolve these shapes in
the viewer's local time, so the same value can bucket into different days for
different people. That's a second reason to store plain `YYYY-MM-DD` strings
(or bridge with `parseEditValue` / `formatEditValue`) rather than relying on
either behaviour.
