...
<Callout type="warning">
  ...
  SQL backends emit — is deliberately **not** accepted. Without a zone it means
  a different day depending on who is looking, and resolving it in the browser's
  local time would disagree with how filtering buckets it. Such a cell opens
  showing its raw text and is rejected on commit with "Use YYYY-MM-DD". Bridge
  it with `parseEditValue` / `formatEditValue`, or store plain `YYYY-MM-DD`
  strings.
</Callout>
