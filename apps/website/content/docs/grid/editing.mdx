...
IME composition takes precedence over editor and grid commands. Enter,
Escape, Tab and arrow events during composition retain browser behavior and
do not save, cancel, step values or start an edit. Automated coverage includes
composition events and the legacy key-code fallback; physical OS IME and human
screen-reader validation are separate from those automated checks.

The built-in fields resolve `components.TextInput` and `components.Textarea`;
number/date action buttons resolve `components.IconButton`. Replacements must
preserve native props, event handlers, ARIA/data hooks, and forwarded refs.
