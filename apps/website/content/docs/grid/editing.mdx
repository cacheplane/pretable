...
| `"date"`                | the text editor, for now                                           |

An `enum` column **without** `options` and every `date` column fall back to the plain text editor — the calendar picker is the next editor to land.
...
- **`renderEditor` replaces the built-in editor** for any column that opens one — text, multi-line, number, the enum combobox, and the date fallback. It does **not** apply to `type: "boolean"`: boolean cells toggle in place and never open an editor, so there is nothing for `renderEditor` to render.
...
On a [typed column](#typed-editors), built-in parsing runs first: by the time `validate` sees the value, a `number` column has already rejected non-numeric drafts ("Not a number") and turned an empty draft into `null`, and an `enum` column with `options` has resolved the typed label to that option's `value` (rejecting anything that matches none) — so `validate` is for domain rules, not parsing.
...
- **Blur commits in place.** Clicking away from an open editor commits the current draft without moving focus. (`Enter` and `Tab` commit _and_ move; blur commits and stays put.) The one exception is the [enum combobox](#enums), where text matching no option reverts on blur instead.
