...
The grouping pane's remaining controls are ordinary form controls — the expansion buttons, the switch, one aggregate picker per row — a select-only combobox, not a native `<select>` — and its **+ Add group** menu is now literally the same keyboard the filters pane's pickers use: arrows move, `Home` / `End` jump to the ends, typing jumps by prefix, `Enter` selects, and `Escape` closes back to the button.
