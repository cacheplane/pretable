---
title: Tool panel
description: "The rail of section tabs at the grid's right edge: the columns section's visibility, pinning, and reorder, and the filters section's AND/OR filter builder."
nav: Grid
---
...
Two sections ship today, and both are documented below: **Columns**, which hides, pins, and reorders columns, and **Filters**, which builds the query's AND/OR filter tree. In the grid below, `defaultActiveSection` opens the columns pane on load — uncheck a row to hide its column, drag a grip to reorder, use the row's ⋮ menu to pin:
...
Add a filter and give it a value, and watch the count under the grid drop; add a group and flip its **and** to **or**; then open the **Sector** funnel and find the row you built already sitting in it:
...
- **Typing is debounced ~200 ms**, the same dwell the funnel menu uses. An operand that committed on every keystroke would rebuild the row model once per character, which on a large grid is a cost you can feel.
- **Every discrete change applies immediately**: operator, date, checklist, the join button, adding a row or a group, removing one. Nothing about those is mid-thought, so nothing waits.
...
The parts expose stable hooks, the same way the [funnel menu](/docs/grid/filtering#the-built-in-menu) does: `data-pretable-filter-row` on a row, with `data-pretable-filter-row-column`, `-operator`, `-value`, and `-remove` on its controls; `data-pretable-filter-join` on the connective (a `<button>` where the join can change, a `<span>` where it cannot); `data-pretable-filter-add` on the two add buttons; `data-pretable-filter-rail` on a nested group's rail; `data-pretable-filter-column-hidden` on a row whose column is hidden; and `data-pretable-filter-empty` on the "no filters" message.

Every string the pane renders comes from the `messages` prop — `toolPanelAddFilterLabel`, `toolPanelAddGroupLabel`, `toolPanelNoFiltersMessage`, the `toolPanelFilter*` labels, and the two refusal sentences — with one exception. The **operator names** in the operator picker are not messages. They are shared verbatim with the header funnel, which is not on the messages layer yet, and localizing them here alone would show one grid's operators in two languages at once. Until the funnel joins, that list is English.
...
The filters pane needs no chords of its own: every control in it is an ordinary form control, one Tab stop each, walked in the order the tree reads — join, column, operator, value, remove, then the add pair, and the same again inside each group. Whichever section is open, the panel is not a trap: the rail tab is the last stop inside it, so one more forward `Tab` from there leaves the grid entirely.

## What's ahead

The rail is built to hold more sections — a grouping pane is planned — and `ToolPanelSectionId` names exactly the ones that ship.
