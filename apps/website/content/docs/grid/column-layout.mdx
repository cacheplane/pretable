...
Columns pin to either edge. Declare it on the column, or set it at runtime:
...
Width, order, and pin are all controlled here — drag a header to reorder it, drag its right-edge handle to resize, double-click the handle to autosize. Symbol is pinned left and Note is pinned right, so dragging a column into either group (or out of one) is visible immediately, and the readout below the grid echoes the committed order, pins, and widths after every drag:

<Example id="column-layout" />

The same pattern — pair each slice's `useState` with its matching `onColumn*Change` callback — is all `columnWidths`, `columnOrder`, and `columnPinned` need, independently of one another.
