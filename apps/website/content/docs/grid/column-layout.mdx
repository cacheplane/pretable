...
Dragging a flex column's resize handle takes it out of the flex set: it keeps the width the drag landed on, exactly as a fixed column would, and the columns that still flex re-share the leftover around it — so the row still ends on the viewport edge as long as one flex column remains and no floor binds.
