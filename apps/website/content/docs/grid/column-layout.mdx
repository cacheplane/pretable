...
Right pinning has no drag affordance — a right-pinned column keeps its pin through a reorder, and pinning to the right edge is a config or `setColumnPinned` call.
...
**Pinned columns are never virtualized away.** Horizontal virtualization only windows the scrollable group, and that window is the viewport minus _both_ pinned widths. A pinned column is in the render snapshot at every scroll position, so a pinned "actions" or identity column can't scroll out from under the user.
