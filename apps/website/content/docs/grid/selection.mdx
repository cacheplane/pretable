...
Check a few orders below, then **Mark shipped** to update just those orders and clear their checkboxes. **Select all orders** and the header checkbox also work: uncheck one order to leave it out of the action. React owns the checked set through `state.rowSelection`; clicking or dragging across cells changes a separate cell-range selection.

<Example id="controlled-row-selection" />

The example subscribes to the public grid handle and echoes `describeRowSelection(grid.getState().selection.rows)` into React state, preserving select-all and exclusions even when `onRowSelectionChange` is silent. It compares descriptions before updating state, so unrelated grid changes do not replace the controlled selection. For the count and bulk action, it checks the three local orders with `grid.isRowSelected(order.id)`. The subscription is removed when the grid changes or the component unmounts.
