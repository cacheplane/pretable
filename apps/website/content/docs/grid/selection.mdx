...
Cell-range selection plus the checkbox column, both controlled from the same `PretableSelectionFor<typeof columns>` — shift-click to extend a range, Cmd/Ctrl-click to add a discontiguous one, drag for a marquee, and watch the caption below the grid echo the active ranges as you go:
