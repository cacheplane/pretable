...
- **Row selection** — the ticked set behind the `rowSelectionColumn` checkbox column, and the thing the second caption is printing. It is stored separately, as a sparse program that can mean "all rows" without ever listing them, so a select-all over a million rows costs nothing. `onRowSelectionChange` reports it.
