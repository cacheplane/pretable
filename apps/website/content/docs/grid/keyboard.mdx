...
- **The grid does not fight your own scrolling.** A focus address that has already been satisfied is never re-scrolled — scroll the focused cell out of view by hand and it stays out of view until focus moves again.
...
<Callout type="note">
  ...
  not exported under a name — take it as `PretableRenderSnapshot["rowMetrics"]`
  if you need to write the type down.
</Callout>
