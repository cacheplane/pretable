...
140 trades, well past the fold, with **ID** pinned left and **Status** pinned right. Focus a cell below and try the jump keys — `Cmd/Ctrl + End`, `PageDown`, `Home` / `End` — and watch the readout track the focus address while the viewport moves the minimum amount needed to reveal it, clear of the header and both pinned groups:
