...
Per-column ReactNode renderer for full UI control. Use it when you need a badge, a button, an icon, or any non-text content — `@pretable/react` ships four ready-made presentations for exactly this: `PretableDelta`, `PretableStatus`, `PretableBadge` and `PretableEntity`, covered in [Cell presentations](/docs/grid/cell-presentations).
