...
export function People({ rows }: { rows: Person[] }) {
  return (
    <Pretable rows={rows} columns={columns} ariaLabel="People directory" />
  );
}
```
