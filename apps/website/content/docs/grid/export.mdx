...
The button below calls exactly that pairing — click it and a real
`products.csv` downloads. Nothing is checked, so `onlySelected` exports
everything; check a few rows first and click again to see it narrow the
file:
