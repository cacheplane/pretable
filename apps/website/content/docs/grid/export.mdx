...
`serializeCsv` turns a row-model snapshot into a CSV file; `defaultSaveFile`
delivers it to the user's disk. They are separate on purpose — the serializer is
pure, so you can upload the result, hand it to a worker, or assert on its bytes
...
Two things there are load-bearing. `<PretableSurface<Position>` needs its type
...
exports everything**, deliberately — a button that silently downloads a zero-row
file is indistinguishable from one that is broken. Pass `rowIds` instead to name
the rows yourself; passing both throws, because they are two ways to say the
same thing and quietly preferring one would drop rows you asked for.
...
| `rowIds`               | `ReadonlySet<PretableRowId>`                | no       | Restrict to these data rows — how selection-only export is expressed.                                    |
