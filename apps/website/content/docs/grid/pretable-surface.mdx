...
`processing` declares whether filter and sort work is performed by the local engine or an external authority; `resultMeta` then describes the result those rows came from, and `dataState` says where its request stands. All of it — query ownership, totals, the lifecycle phases, and `renderBodyState` — is documented against a live endpoint in [Server-side data](/docs/server-data).
