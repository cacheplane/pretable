...
| `state`                                         | `PretableSurfaceState \| null` &nbsp; _(@experimental)_     | no       | Slice-based controlled state injection: `{ sort?, filters?, selection?, focus? }`. Controlled focus uses `{ ref: { kind: "data", rowId } \| { kind: "group", groupId } \| null, columnId }`, so data and group identities never collide. Each slice is independently controlled — provide it to force the engine, omit to let the engine own that slice. |
...
| `onFocusChange`                                 | `(next: PretableSurfaceFocusState) => void`                 | no       | Fired on user-induced focus changes with a discriminated data/group row `ref`. Same controlled-mode pattern as `onSelectionChange`.                                                                                                                                                                                                                      |
