...
So both this and `PretableStatus` show a dot, and the chip is what separates
them: a badge is a noun the row _is_ — a category, a flag — and reads as a
bounded chip; a status is a state the row is _in_, and reads as a dot beside
plain text.
