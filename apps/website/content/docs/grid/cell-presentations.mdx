...
The focused examples below use the same shared theme and grid styles. Import
them once in your app as shown in [Import the styles](/docs#import-the-styles).
Each Code tab contains a complete component you can copy.
...
display text as `children` — the focused example uses `Intl.NumberFormat`. In
the combined grid, `formattedValue` is the string the column's `numberFormat`
already produced.
...
<Example id="presentation-status" />
...
<Example id="presentation-entity" />
