...
const res = await fetch("/api/events");
connectElementStream(rowModel, parseElementStream<Row>(res.body));
```
...
const res = await fetch("/api/single-row-events");
connectPartialStream(rowModel, parsePartialStream<Row>(res.body), {
  rowId: "row-001",
});
```
...
```ts
const conn = connectElementStream(rowModel, parseElementStream(res.body));
