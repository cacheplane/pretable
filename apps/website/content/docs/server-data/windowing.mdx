---
title: Windowing
description: "resultMeta.window says where the loaded rows sit inside the result, telemetry.windowGap says when the reader has run off the end of them, and one rule keeps the positions the grid announces from ever contradicting the count."
...
## A gap you may not be told about

`windowGap` has a known false-negative, and it is documented here rather than left to be discovered.

The row layout controller replans on a scroll, a viewport change, a column change, or a row-model change — deliberately not on a `resultMeta`-only change, which it does not read. The gap check compares row counts derived fresh from `resultMeta` every render against a total height that came from the last plan. While the total **grows**, the mismatch only makes the boundary more permissive, so a viewport already past the window keeps reading as past it. While it **shrinks**, the same arithmetic goes the other way: `windowGap` can report `undefined` for a viewport that a fresh plan would still call past the loaded window.

It corrects on the next replan, which any scroll or row change triggers — so the case that reaches a reader is narrow: a `resultMeta` update that lowers `total.count` while the rows and the viewport both hold still, with the reader parked past the edge and not moving. Nothing recovers it until they do move.

This is pinned by a test rather than papered over, because the fix is not local: it means changing when the row layout controller replans, and that controller is kept ignorant of `resultMeta` on purpose. Reopening that is its own decision, not a detail of this feature.

The same seam has a mount-time twin, pointing the other way. Between your first block committing and the row layout first running, the plan's total height is still `0`, which puts the end of the loaded window at a negative pixel and makes any viewport past it: two `"after"` gaps are reported for a grid nobody has scrolled. They stop at the first real layout, and a handler that acted on them has already spent two requests at the moment the page is busiest. The example above ignores any gap whose telemetry still reports `totalHeight: 0` for exactly that reason, and it is the one line of it worth copying verbatim.

The practical rule is to treat an absent gap as "no signal", never as proof that the viewport is inside the window. If your own state can tell that the reader is past the loaded block — a total that just shrank under them, say — act on what you know.
