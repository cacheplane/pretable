...
<Callout type="note">
  Groups now reach your endpoint from the UI, so this is not a theoretical shape
  to plan for. The [tool panel](/docs/grid/tool-panel)'s filters section ships a
  `+ group` action at every level of the tree, and every group it builds is
  submitted through `query`/`onQueryChange` like any other filter — as are
  groups you seed yourself through the controlled `query` prop, restore from a
  URL, or load from a saved view. The header funnel is unchanged: it writes,
  edits, and removes its own column's top-level leaf and nothing else. If your
  backend cannot answer a nested query yet, pick one of the three answers above
  deliberately — **reject** is the one that stays honest while you decide.
</Callout>
