...
- **Interaction proof (H1 + H6–H12) — comprehensive.** `status/milestones/2026-05-01-interaction-comprehensive.hypotheses.json`: at `{S2, S7}/hypothesis/{scroll, sort, filter-metadata, filter-text}` × 3 repeats × {pretable, ag-grid, tanstack}. Verdicts: **H1 satisfied**, **H7 satisfied** (S2 metadata-filter), **H8 satisfied** (S2 text-filter), **H9 satisfied** (S7 scroll), **H10 satisfied** (S7 sort), **H11 satisfied** (S7 metadata-filter), **H12 satisfied** (S7 text-filter). **H6 directional** (S2 sort) — current medians within thresholds but repeat-variance keeps it from satisfied; S7 sort (H10) is satisfied at 8.4ms median interaction latency, so the path to H6 satisfaction is variance reduction, not feature work.
...
- H6 (S2 sort) is directional rather than satisfied as of 2026-05-01 — repeat-variance pushed the median over the latency threshold even though S7 sort (H10) satisfies cleanly at the same scale. Likely an outlier repeat; worth re-running before product moves rely on H6 being satisfied.
- S3 column virtualization does not currently have a committed milestone runset — the historical proof referenced was on a gitignored runset that no longer exists on disk.
