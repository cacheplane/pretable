...
1. **Receipts** (`#receipts`) — the headline numbers: 4× scroll vs Grid Alpha, zero row-height-error, H15 satisfied. Same ReceiptsBand we have today, pruned.
...
- `<Problem>` — merged into Receipts (single line: "Grid Alpha clips wrapped content to one line at hypothesis scale")
...
- **Comparison table** — each adapter row gets a trail marker. `pretable` = green-circle (recommended path), `gridalpha` = blue-square (familiar but slower), `gridbeta` = black-diamond (powerful but DIY), `gridgamma-x` = double-black-diamond with caution chevron (broken at scale)
