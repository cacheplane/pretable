...
- **Aligned with modern library practice**: TanStack-style changesets workflow, dual ESM+CJS, publint+attw verification, npm provenance
...
Aligns with TanStack Query's per-package `test:build` pattern. Each public package gets a `lint:packaging` script:
...
1. **Changelog generator** — `@svitejs/changesets-changelog-github-compact` auto-links commits and PRs in GitHub-flavored style. Matches TanStack and Radix.
