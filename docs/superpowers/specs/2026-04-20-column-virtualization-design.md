...
All four adapters (pretable, AG Grid, TanStack, MUI) already handle arbitrary column counts from the dataset. AG Grid and TanStack have built-in column virtualization. MUI virtualizes rows but not columns. No adapter code changes needed.
...
5. S3 comparative: `pnpm bench:matrix -- --project=chromium --adapters=pretable,ag-grid,tanstack,mui --scenarios=S3 --scripts=scroll --scale=hypothesis --repeats=3`
