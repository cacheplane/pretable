...
2. No competitor uses **warm amber accent**. GridBeta owns cyan; Grid Alpha and GridGamma use cool blues. We own `#8a5d0f` on cream / `#c68a1e` on dark.
3. No competitor **contrasts editorial light chrome against dark terminal product surfaces**. GridGamma and Grid Alpha are uniformly light-doc; GridBeta is uniformly dark-dev. We bracket a dark grid between two cream bands.
...
5. Grid Alpha has no live benchmark page. We make the bench our second surface.
...
4. **Receipts band (cream)** — 52px padding. "_Receipts_, not claims." H2 with italic amber. 4-column grid of big Fraunces numbers (`500k`, `2.4ms`, `0`, `4.1× gridalpha`) with top hairline rules + caption ("on S7 stress scenario, 2 pinned"). Link to bench: "See them re-run in the bench →".
5. **Footer** — shared.
...
3. **Control strip (cream-hi)** — segmented pills for scenario (`S1` / `S2` / `S7`), scale (`tiny` / `dev` / `stress`), adapters (multi-select: `pretable` / `gridalpha` / `gridbeta` / `gridgamma-x`). Amber-fill on active state. Live permalink `/bench?s=S2&v=all&scale=dev` auto-updates as you click. Run button right-aligned with green status dot.
...
**Permalink as conversion vector:** `/bench?s=S7&v=pretable,gridalpha&scale=stress` — senior engineer pastes into Slack, team clicks, they see the exact result. This is the "staff eng shares with VP" moment.
...
- Left (260px region): `pretable.` wordmark + amber period + `v0.4` pill (GridGamma-style)
