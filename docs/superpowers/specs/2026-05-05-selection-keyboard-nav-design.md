...
2. **B2 — comparative bench** for selection/nav (vs AG Grid / TanStack / MUI X). Selection-model parity makes this its own engineering project.
...
- A range with a row currently filtered out is kept in state, but contributes zero cells to copy / "selected rows" derivations until the row reappears. Matches AG Grid.
...
`exit` matches strict ARIA (Tab leaves the grid). `wrap-rows` matches AG Grid / Sheets convention. Default is `wrap-rows` because pretable is a data-grid product, not an accessibility-purist component library.

`Cmd+arrow` does not implement Excel's "data block" semantics — every typed cell has a value. Jumping to grid edge is the predictable behavior and matches AG Grid.
...
Announcement strings are exposed via a `messages?` prop for i18n (matching AG Grid's pattern). English defaults ship in v1.
