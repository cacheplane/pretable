...
Quantify how each grid adapter degrades as streaming load climbs from the H13 baseline (1k patches/sec) through high-throughput regimes (25k patches/sec). The H13 phase 2 finding (PR #23) was that at 1k/sec Pretable matches Grid Alpha and GridBeta on frame p95, only decisively beating GridGamma X Community. This memo asks: **at what load does each adapter actually break, and does Pretable's wedge widen at higher rates?**
...
- **Adapters:** Pretable, Grid Alpha Community, GridBeta Virtual, GridGamma X Community.
...
- **Streaming pattern per adapter** (idiomatic, see PRs #15, #23):
  ...
  - Grid Alpha: `gridApi.applyTransaction({ update })` (native batched).
  - GridGamma X Community: `apiRef.updateRows([row])` looped per patch (Community caps `updateRows` to one row per call; batched is Pro/Premium-only).
  - GridBeta Virtual: `setRows` merge — full reconciliation per batch.
...
Pretable, Grid Alpha, and GridBeta are essentially tied on frame p95 across the entire 100–25,000 patches/sec range — all in a 9.7–10.1 ms band, well under the 16 ms 60 Hz budget. **Pretable does not have a numeric speed advantage** at any tested rate. The streaming-demo's 9 ms p95 claim (PR #16) is true but is matched by both Grid Alpha and GridBeta on the same scenario.

### The decisive comparative wins are vs GridGamma X Community

GridGamma X Community starts breaking at **500 patches/sec** and is unusable at any meaningful rate. Its `updateRows` is capped to one row per call in Community (Pro/Premium-only batched), and the 50× overhead per batch accumulates blocking work faster than the frame loop can clear. By 5,000 patches/sec, the page is unresponsive 9× longer than the test runs.

This is a real win, but it's a **win against a hobbled tier**, not against the field.

### Beyond p95: small differences that add up

- **Frame max:** Pretable + Grid Alpha stay at ~10.4 ms (one frame budget). GridBeta consistently has one frame at 16.1–16.7 ms — right at the edge. So Pretable + Grid Alpha are more **predictable** than GridBeta at the worst-frame level.
- **Visible-row count drift:** Grid Alpha drifts by 22–28 rows at low rates, settling to 0 at high rates. Pretable + GridBeta stay at ≤ 1 across all rates. Grid Alpha is doing something with row recycling at low rates that the others don't — visible behavior, not a frame budget issue, but a stability signal.
- **CLS:** Grid Alpha has zero. The other three have a small steady ~0.0007. Below human-perceptible thresholds for all four.
...
- Pretable, Grid Alpha, and GridBeta are streaming-capable on Chromium at hypothesis scale; all stay under 16 ms p95 from 100 to 25,000 patches/sec.
- GridGamma X Community is not streaming-capable above ~100 patches/sec.
- Grid Alpha recycles rows visibly at low rates (22–28 row drift); Pretable does not.

## Strategic implications

### Should H13 change?

H13 currently asks Pretable to match the best comparator within 10% parity at 1,000 patches/sec, with at least one comparator failing absolutes. That's still **satisfied** — Pretable and Grid Alpha/GridBeta tie within 10%, and GridGamma fails absolutes.
...
> **H14 — Streaming Operating Envelope.** Pretable handles streaming updates from 100 to 25,000 patches/sec on S5 hypothesis scale without exceeding the 60 Hz frame budget (frame p95 ≤ 16 ms, zero long tasks). At least one tested comparator (GridGamma X Community) fails this envelope at ≥ 500 patches/sec.

This is an honest claim. It's also true for Grid Alpha and GridBeta — but the wedge sentence is "Pretable handles X without breaking" not "Pretable beats Y." It's a **defensive** claim, not an offensive one. That's fine. The streaming-demo + integrated stream-adapter is a viable streaming target.

### H15 proposal: row-stability under streaming

> **H15 — Row-stability under streaming.** Pretable's visible-row count remains constant during streaming updates (drift ≤ 1 across the operating envelope). Grid Alpha drifts by 22–28 rows at sub-5k rates.

This is a real differentiator vs Grid Alpha specifically. It's not a wedge against GridBeta (which also stays at 0), but it's specific, measurable, and visible to users (less row-shuffling means less visual disturbance during streaming).
...
1. **Purpose-built streaming integration.** `@cacheplane/json-stream` + `@pretable-internal/stream-adapter` is the only adapter with a documented streaming pipeline. Grid Alpha + GridBeta require you to wire your own.
2. **Predictable frame budget.** `frame_max ≈ 10.4 ms` consistently, vs GridBeta's 16-17 ms at the edge.
3. **Zero visible-row drift.** Grid Alpha's drift is real even if invisible to a casual viewer.
4. **No fall-apart mode.** GridGamma X Community shows what happens when an adapter isn't built for streaming. Pretable, Grid Alpha, and GridBeta all avoid this; not a unique win, but a shared baseline.
...
- Consider Pro/Premium tier comparators (GridGamma X Pro batched updateRows, Grid Alpha Enterprise) — they may put up a fight that Community cannot.
