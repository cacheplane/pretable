...
Pretable's scroll performance wedge is proven: H1 (comparative scroll vs Grid Alpha, GridBeta, GridGamma) and H3 (scroll stability — blank gaps, anchor shift) are satisfied on repeated Chromium S2/hypothesis runs.
...
**Verification:** `pnpm bench:matrix -- --project=chromium --adapters=pretable,gridalpha,gridbeta,gridgamma --scenarios=S2 --scripts=scroll --scale=hypothesis --repeats=3`
