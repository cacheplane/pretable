...
Let `pnpm bench:matrix` include adapters and scripts that are not supported together (for example `gridalpha` × `sort`) in a single invocation without failing, by having the Playwright bench spec persist an `unsupported` summary for every executed combination.

## Current Behavior

`pnpm bench:matrix -- --adapters=pretable,gridalpha,gridbeta --scripts=scroll,sort,filter-metadata,filter-text --scale=hypothesis --repeats=3` fails with:

```
Error: bench:e2e did not write a summary for gridalpha/S2/sort#1
...
After the fix, `pnpm bench:matrix -- --adapters=pretable,gridalpha,gridbeta --scripts=scroll,sort,filter-metadata,filter-text --scale=hypothesis --repeats=3` produces a single canonical runset covering:

- `pretable` × `{scroll, sort, filter-metadata, filter-text}` — measured
- `gridalpha` × `scroll` — measured
- `gridalpha` × `{sort, filter-metadata, filter-text}` — unsupported
- `gridbeta` × `scroll` — measured
- `gridbeta` × `{sort, filter-metadata, filter-text}` — unsupported
...
This is the primary red-green gate. Run the spec directly against an unsupported env combination (for example `PRETABLE_BENCH_ADAPTER=gridalpha PRETABLE_BENCH_SCRIPT=sort PRETABLE_BENCH_SCENARIO=S2 PRETABLE_BENCH_SCALE=hypothesis`) to observe the red state before editing the spec body.
...
2. E2E bench matrix:
   - `pnpm bench:matrix -- --project=chromium --adapters=pretable,gridalpha,gridbeta --scenarios=S2 --scripts=scroll,sort,filter-metadata,filter-text --scale=hypothesis --repeats=3`
