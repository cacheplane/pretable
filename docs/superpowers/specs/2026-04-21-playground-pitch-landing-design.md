...
- 4-column grid (`grid-cols-4`, gap ~24px):
  ...
  - Initial values: `{value: "500k", caption: "rows rendered"}`, `{value: "2.4ms", caption: "frame p50"}`, `{value: "0", caption: "jank events"}`, `{value: "4.1×", caption: "vs ag-grid"}`. Replace during implementation with numbers pulled from a real bench run on the local machine; values above are spec placeholders.
