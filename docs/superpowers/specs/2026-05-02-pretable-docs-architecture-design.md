...
Topical IA with a Diátaxis-hybrid shape per section: each top-level section follows **Concept → Recipes → Reference**. This pattern matches the user's other repos (`angular-agent-framework`, `dawn`) and aligns with field conventions (Grid Alpha's per-feature concept→reference depth, Tailwind's "concept hub + token reference + recipes + escape hatch", shadcn's CSS-variable contract pattern).
...
- **Hand-authored MDX.** No auto-generation from TypeScript for v0.0.x. The 25-page surface is small enough to maintain by hand; matches `dawn`'s pattern over `angular-agent-framework`'s scale-driven Assistant-API generation.
...
| Assistant-API narrative auto-gen | YAGNI for hand-authored docs | docs at scale (100+ pages) |
...
Diverges from:
- **Grid Alpha** — too deep (~150 pages) for pretable's pre-1.0 surface
- **GridGamma X** — fragmented theming docs (no token reference); pretable explicitly avoids this
- **GridBeta Table** — pure Diátaxis with Guides ↔ APIs mirroring; pretable uses topical-with-embedded-reference instead

## Appendix B: cited sources

- User's repos: `/Users/blove/repos/angular-agent-framework`, `/Users/blove/repos/dawn`
- Grid Alpha docs: https://www.gridalpha.com/react-data-grid/
- GridBeta Table docs: https://gridbeta.com/table/latest/docs/introduction
- GridGamma X DataGrid docs: https://gridgamma.com/x/react-data-grid/
