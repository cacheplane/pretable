...
12. **"composite H1 satisfied with MUI-like competitor failing row height accuracy"** — Pretable 24ms, MUI 25ms, pretable quality all zero, MUI `row_height_error_p95_px: 1.1`. Frame parity = 96%. Uniqueness holds. Assert H1 `satisfied`.
...
2. Full matrix run: `pnpm bench:matrix -- --project=chromium --adapters=pretable,ag-grid,tanstack,mui --scenarios=S2 --scripts=scroll --scale=hypothesis --repeats=3`
3. Inspect the resulting `*.hypotheses.json`:
   - H1 status should be `satisfied` (pretable: 0 artifacts, MUI: 1.1px height error fails uniqueness gate)
   - No H3 entry exists
   - H5 still `satisfied`
   - 5 hypotheses total
...
The 1px row-height-error threshold is tight — MUI currently measures 1.109375px, so a minor MUI update could push it under 1px and break the uniqueness gate. This is acceptable: if MUI achieves sub-pixel accuracy, that's genuine progress and pretable's uniqueness claim should honestly reflect it. The threshold should not be loosened to preserve a failing claim.
