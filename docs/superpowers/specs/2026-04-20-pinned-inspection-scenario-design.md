...
All four adapters (pretable, AG Grid, TanStack, MUI) already respect `column.pinned` from the dataset and handle variable-height rows. S7 feeds them different column config through the same `ScenarioDataset` interface.
...
3. Hypothesis-scale comparative: `pnpm bench:matrix -- --project=chromium --adapters=pretable,ag-grid,tanstack,mui --scenarios=S7 --scripts=scroll --scale=hypothesis --repeats=3`
