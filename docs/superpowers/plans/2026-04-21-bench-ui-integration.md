...
Boot the dev server, walk through the bench workflow, and fix any visual regressions the token swap introduced. Also check for vendor-CSS conflicts (Grid Alpha, GridGamma X) and apply the adapter-scoping fallback if needed.
...
- **Try each adapter** (`?v=gridalpha`, `?v=gridbeta`, `?v=gridgamma`, `?v=pretable`). Each adapter's own widget renders inside `.viewport-card` without breaking layout.
...
Grid Alpha and GridGamma X ship their own CSS. Likely outcomes, in order of likelihood:

1. **No conflict** — vendors' styles are scoped tightly. No action. Move to Step 3.
2. **Minor cosmetic drift** — vendor widgets look subtly off against cream chrome (default fonts, hover colors). Tolerate it. Log in Step 3's commit message as "known: gridalpha/gridgamma vendor style drift, acceptable."
...
- If vendor CSS (Grid Alpha, GridGamma X) shows cosmetic drift against the cream chrome in manual testing, future tightening is a small follow-up.
