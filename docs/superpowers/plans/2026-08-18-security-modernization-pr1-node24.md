...
Create a unique directory with `mktemp -d`, then run `pnpm --filter <package> pack --pack-destination <literal-temp-dir>` for core, react, stream-adapter, and UI. Inspect each tarball with `tar -tf`; require manifests, README/license where currently shipped, `dist` ESM/CJS/declarations, and UI CSS assets. Reject workspace source, private package names, tests, configs, and nested `node_modules`.
