...
This test (starts at line 238 before Task 1 edits) has a pretable run at line 310-329 that is missing quality metrics (`row_height_error_p95_px`, `scroll_anchor_shift_backward_p95_px`). The composite H1 needs these. Also, the gridalpha run already has `row_height_error_p95_px: 0.2` and `scroll_anchor_shift_px: 8` — under composite H1, gridalpha passes all quality sub-criteria, so uniqueness fails. We need gridalpha to fail at least one.
...
Update the gridalpha run's `row_height_error_p95_px` from `0.2` to `2` so it fails the 1px quality threshold (uniqueness gate holds):
...
The gridalpha runs in this test (around lines 641-660) have `row_height_error_p95_px` values of 42-45 — these fail the 1px threshold, so uniqueness holds. The pretable runs use `createScrollRun` which defaults quality metrics to 0. H1 should still be `satisfied`.
...
This test has pretable at 12.4ms, gridalpha at 26.2ms, gridbeta at 20.1ms. Under composite H1, pretable passes all quality thresholds (defaults to 0). Grid Alpha has `row_height_error_p95_px: 0.2` — this passes the 1px threshold. But `scroll_anchor_shift_px: 8` — this also passes the 16px threshold. Grid Alpha passes all quality sub-criteria, so uniqueness fails. We need gridalpha to fail at least one.

Update the gridalpha run (around line 880) to have `row_height_error_p95_px: 2`:
...
- [ ] **Step 4: Add test — composite H1 satisfied with GridGamma-like competitor**
...
uniqueness gate, GridGamma-like competitor, anchor shift check, and
...
Run: `pnpm bench:matrix -- --project=chromium --adapters=pretable,gridgamma --scenarios=S2 --scripts=scroll --scale=dev`
