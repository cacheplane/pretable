...
- [ ] **Step 12: Quality review fixes**

A second review on `e5dd96cf` ran against the rebuilt, running app (not just jsdom) and found one shipping blocker plus fifteen further issues. All fixed in one follow-up commit.

**Blocker — Code view unreachable by keyboard.** Roving tabindex was applied to the Preview/Code tabs, but the arrow-key handler was wired only to the file tabs, so the unselected view tab was neither Tab-reachable nor arrow-reachable — a keyboard-only reader could not open the source on a component whose entire purpose is showing source. Fixed by extracting the roving-tabindex arithmetic (`nextTabIndex`) into a function shared by both tablists, adding Home/End (APG expects all four, not just Left/Right), and wiring `onKeyDown` to the view tabs. Verified on the rebuilt running build (`pnpm build`, `pnpm exec next start`, driven with the browser tool, not jsdom): focused the Preview tab, dispatched a real `ArrowRight` key press, and confirmed focus and `aria-selected` moved to the Code tab. Mutation-proven: removing the `onKeyDown` wiring from both view-tab buttons fails exactly the two new keyboard tests (`moves view-tab selection with the right arrow key, reaching Code` and `...with Home and End`); reverted after confirming.

**Important:**
1. **Colliding DOM ids across multiple examples on one page.** Literal ids (`example-tab-preview`, etc.) meant a second `<Example>` on a page resolved every `aria-labelledby`/`aria-controls` — via `getElementById` — to the *first* example's elements. Fixed with `useId()`, SSR-stable in React 19. New test renders two `<ExampleShell>`s and asserts each tab's `id` is unique and each `aria-controls` panel is labelled by that same tab, not the other instance's.
2. **Unguarded `navigator.clipboard.writeText`.** In a non-secure context (checking docs over `http://<lan-ip>:3000` on a phone) `navigator.clipboard` is `undefined`; a rejected `writeText` (Safari/Firefox) behaves the same. Either way `copy()` threw inside an unawaited async function — unhandled rejection, silent no-op on a headline feature. Wrapped in try/catch; a failure sets a distinct `${label}-failed` state rendered as "Copy failed", never silence. Two new tests cover the missing-API and rejected-promise cases.
3. **Unowned 2-second reset timer.** No ref, no clear, no unmount cleanup — clicking Copy file then Copy for agent 1.9s later let the first timer fire late and wipe the second button's "Copied" out from under it (`copied` is one shared piece of state). Fixed: the timeout id lives in a ref, cleared at the top of every `copy()` call and on unmount. New test reproduces the exact repro from review (click file, wait 1.9s, click agent, wait to the first timer's original fire time, assert "Copied" survives) using fake timers.
4. **Shiki's inline `background-color` beat the CSS rule.** `.pretable-example-code pre { background: transparent }` in globals.css was dead: Shiki bakes `style="background-color:#fff;..."` onto every `<pre>`, and inline style beats any class, `!important` included — the code pane painted white on the warm `#fff8f1` card. Fixed at generation time in `lib/docs/examples/load.ts` with a Shiki `pre(node)` transformer that strips just the `background-color` property, keeping the CSS rule honest instead of escalating to `!important` (which would only win one specificity tier, not end the fight). New regression test in `examples-load.test.ts` asserts the `<pre>` tag's inline style has no `background-color`. Mutation-proven: removing the transformer fails exactly that test.
5. **The homepage streaming demo finished before anyone saw it.** Both panes have always stayed mounted (by design — see the layout comment), so `MockChatGrid`'s `setInterval` (4 rows × 700ms) ran to completion inside the *hidden* preview pane from page load, before a reader ever clicked Preview — under a heading that says "Selection survives every chunk." Same root cause as issue 6 below: fixed together with a one-shot mount latch.
6. **Every demo mounted and ran at page load regardless of viewport or view.** N live grids doing layout, `ResizeObserver`, and timer work while invisible. Fixed with `useInView` (already shipped in `app/components/showcase/useInView.ts`) plus a `demoMounted` latch that becomes true the first time the pane is both scrolled into view *and* selected, computed during render (`if (!demoMounted && inView && view === "preview") setDemoMounted(true)`) rather than in a `useEffect` — this is React's documented "store information from previous renders" pattern, not a synchronization effect, and avoids the `react-hooks/set-state-in-effect` lint error a naive `useEffect` version hit. `{demoMounted && children}` replaces the unconditional `{children}`. The layout comment gained a note that React 19.2's `<Activity mode="hidden">` is *not* a substitute — it hides via `display: none` and unmounts effects, exactly the two hazards this file works around. Verified on the rebuilt running build: opened the drawer, scrolled the homepage's streaming-chat-grid example into view while it was still on Code (its `initial` view) — the preview pane had no `<table>` at all, proving the demo hadn't started. Clicking Preview and taking two screenshots ~1s apart showed 2 rows, then 3–4 rows — genuinely animating live, not arriving finished.

**Minor:**
7. `border-rule-strong` wasn't a real token (the site's `@theme inline` block has no `--color-rule-strong`; the served stylesheet had zero occurrences) — the active file tab's underline only looked right because it fell back to `currentColor`, which happened to already be `text-text-primary`. Replaced with the existing `border-text-primary` token, pinning the same visual result explicitly instead of by accident.
8. No test asserted the faded-pane technique (`inert` + `aria-hidden`) that keeps the hidden pane out of find-in-page and the a11y tree. New test clicks Code and asserts the preview pane gets `inert`/`aria-hidden="true"` while the code pane loses both, and the converse on load. Mutation-proven: removing both `inert` attributes fails exactly this test.
9. `Example.test.tsx` never proved `agentMarkdown` (built by the real `toMarkdown(example)`) reaches a reader-facing surface. New test stubs the clipboard, clicks Copy for agent on the real `streaming-chat-grid` example, and asserts the captured text contains `Source: https://pretable.ai/examples/streaming-chat-grid.md` — the only test that proves the `.md` URL convention reaches a reader, not just a unit under test.
10. `CodeExample.test.tsx`'s stub comment pointed at `ExampleShell.test.tsx`; updated to point at `Example.test.tsx`, which now owns the real-pipeline coverage.
11. `files[active]` was unclamped — if `ExampleShell` were ever reconciled into a different example at the same tree position, `active` would survive the swap while `files` didn't, and `file.html` could throw. `Example.tsx` now passes `key={id}` so a different id remounts instead of reusing state that no longer matches.
12. Making an ancestor `inert` blurs focus inside it to `document.body` — a reader who clicked Code while focus was inside the demo grid would lose their place with no way back in via keyboard. `selectView`/`selectFile` now explicitly `.focus()` the newly-selected tab (matches the APG tab pattern anyway, and covers Safari, which doesn't focus buttons on click by default). New test asserts `document.activeElement` after a click.
13. The code pane was missing `transition-opacity` (present on the preview pane), so one direction faded and the other snapped. Added for symmetry.
14. Added the two "why" comments this codebase documents scrupulously elsewhere, both in `Example.tsx`: why the demo crosses the server/client boundary as a rendered `children` element rather than passing the `Demo` component type as a prop (React can serialize an already-rendered Server Component tree across the RSC boundary, but not a component type — a Client Component can't call a bare function reference handed to it from the server), and why the `files.map()` re-shape drops `LoadedFile.focusLines` from the client payload (it only exists to compute `.line-focus` classes into `html` at load time; the shell never reads it, so shipping it would be dead weight in every page's RSC payload).
15. At 375px both tab strips wrapped to two lines and long filenames broke mid-word. Added `overflow-x-auto whitespace-nowrap` (with `flex-nowrap`/`shrink-0` on the surrounding flex containers) so both strips scroll horizontally instead. Verified visually at a 375px viewport: both strips stayed one line, `.md` scrolled off the right edge instead of wrapping, and `GroupingPanelGrid.tsx` stayed intact. Also measured the dimmed-line contrast (`globals.css`, unfocused `.line`s inside a file with `.line-focus`): at the original `opacity: 0.42`, the github-light theme's dominant near-black body text (`#24292e`) landed at 2.44:1 against the card background (`#fff8f1`) — well under WCAG AA's 4.5:1. Raised to `0.65`, bringing that dominant color to 4.61:1. The theme's more saturated single-purpose tokens (keyword red `#d73a49` at 2.63:1, identifier blue `#005cc5` at 3.05:1) still fall short at any opacity that still reads as "dimmed" — those hues are inherently low-contrast against a warm background regardless of opacity, a known, shared limitation of "fade the unfocused code" UIs generally. Documented both the measurement and the residual gap in a `globals.css` comment rather than claiming full compliance.

Run from `apps/website`: `pnpm test`, `pnpm typecheck`, `pnpm lint`, `pnpm exec prettier --check` on touched files, `pnpm build`.
Expected: all green; `pnpm build` succeeding is what makes the browser verification above possible.

```bash
git add apps/website/__tests__/components/CodeExample.test.tsx \
        apps/website/app/components/docs/mdx/Example.tsx \
        apps/website/app/components/docs/mdx/ExampleShell.tsx \
        apps/website/app/components/docs/mdx/__tests__/Example.test.tsx \
        apps/website/app/components/docs/mdx/__tests__/ExampleShell.test.tsx \
        ...
        apps/website/lib/docs/__tests__/examples-load.test.ts \
        apps/website/lib/docs/examples/load.ts
git commit -m "fix(docs): keyboard-reachable Code tab, id collisions, clipboard guards, deferred demo mount"
```
