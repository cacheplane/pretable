...
The number handler uses a state machine approach for streaming validation. Instead of accumulating the full buffer and validating with a regex at the end (like parserbaseline), it validates format constraints as each character arrives. This allows early rejection of `01`, `00`, etc.
