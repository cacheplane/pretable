...
In the existing `<thead>` block, replace each `<th>` cell that contains an adapter name (e.g., the cell that says "ag-grid") with:
...
Apply the same pattern to the other three adapter columns (pretable / tanstack / mui-x), using the variants in the table above.
