...
Expected: no matches **other than** the comparator adapter wiring (GridBeta/Grid Alpha/GridGamma handlers, which use their own per-grid APIs and do not touch `@pretable/*`). If matches appear in `pretable-adapter.tsx`, update them as in Task 6.
...
- Shift+click: if `lastCheckedRowAnchorRef.current` exists and is in current visible rows, walk from anchor row to clicked row (inclusive, regardless of order), and for each row in that span, ensure it's selected (toggle ON if not already a full-row range — match Grid Alpha: shift+click "extends selection ON", doesn't toggle individually). Update `lastCheckedRowAnchorRef.current = rowId` after.
