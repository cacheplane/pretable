...
- **Ratio vs ag-grid** or next-best competitor (e.g., `4.1×`)

If any metric can't be sourced today (e.g., ag-grid comparison not yet wired up in the bench matrix), mark the caption with an inline `(n/a)` or substitute a metric that **is** sourceable (e.g., `0` scroll jank on S7 → caption `scroll jank @ S7`).
...
const STATS: readonly Stat[] = [
  ...
  { value: "<real-value-4>", caption: "vs ag-grid" },
];
```
