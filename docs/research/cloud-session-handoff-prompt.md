...
- `H1` (wrapped-text scroll win vs DOM competitor): **satisfied** — Pretable `scroll_frame_p95_ms` 24.9ms vs Grid Alpha 41.6ms (~40% margin, past the 25% bar), GridBeta 24.8ms. 0 blank gaps and 0 long tasks across all three.
...
- Scroll S2 `scroll_frame_p95_ms` medians: Pretable 24.9ms, Grid Alpha 41.6ms, GridBeta 24.8ms. 0 blank gaps, 0 long tasks for all three adapters.
- DOM peak nodes: Pretable 540, GridBeta 540, Grid Alpha 657.
...
- the wrapped-text scroll comparison against Grid Alpha and GridBeta is satisfied at hypothesis scale
